...
def test_data_sources():
    ...
    assert np.isin(["cases", "hosps"], CovidData(include = ["cases", "hosps"]).data.columns.to_list()).all()
...
def test_geo_values():
    ...
    assert np.isin(["US", "42"], CovidData(geo_values = ["US", "42"]).data.location.unique()).all()
